    ...
    return (
      ...
      >
        <input
          type={type}
          className={cn(
            `shadow-input dark:placeholder-text-neutral-600 flex h-10 w-full rounded-md border-none bg-zinc-800 px-3 py-2 text-sm text-white transition duration-400 group-hover/input:shadow-none file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-neutral-400 focus-visible:ring-[2px] focus-visible:ring-orange-500 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:bg-zinc-800 dark:text-white dark:shadow-[0px_0px_1px_1px_#404040] dark:focus-visible:ring-orange-500`,
            className
          )}
          ref={ref}
          {...props}
...
const AnimatedForm = memo(function AnimatedForm({
  ...
  const validateField = (
    ...
    if (isRequired && !value.trim()) {
      ...
      newErrors[fieldName] = "Password must be at least 6 characters long";
    } else {
  ...
  const handleFieldChange = (
    ...
    validateField(field.label, value, field.type, field.required);
    ...
    <section className="max-md:w-full flex flex-col gap-4 w-96 mx-auto">
      ...
      {googleLogin && (
        <>
          ...
          >
            <button
              className="g-button group/btn bg-transparent w-full rounded-md border h-10 font-medium outline-hidden hover:cursor-pointer hover:border-orange-500 hover:bg-orange-500/10 transition-all duration-300"
              type="button"
          ...
          <BoxReveal boxColor="var(--skeleton)" duration={0.3} width="100%">
            <section className="flex items-center gap-4">
              <hr className="flex-1 border-1 border-dashed   border-neutral-800 dark:border-neutral-200" />
              <p className="text-neutral-00 dark:text-neutral-200 text-sm ">
                or
              </p>
              <hr className="flex-1 border-1 border-dashed   border-neutral-800 dark:border-neutral-200" />
            </section>
      ...
      <form onSubmit={handleSubmit}>
              ...
              >
                <section className="relative">
                  <Input
                    ...
                    className={errors[field.label] ? "ring-2 ring-red-500" : ""}
                  />
        ...
        >
          <button
            className="bg-gradient-to-br relative group/btn from-orange-500 to-orange-600
            hover:from-orange-600 hover:to-orange-700 block w-full text-white
            rounded-md h-10 font-medium shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset] 
              outline-hidden hover:cursor-pointer transition-all duration-300"
            type="submit"
          >
            {submitButton} &rarr;
            <BottomGradient />
        ...
        {textVariantButton && goTo && (
          <BoxReveal boxColor="var(--skeleton)" duration={0.3}>
            <section className="mt-4 text-center hover:cursor-pointer">
              <button
                className="text-sm hover:cursor-pointer outline-hidden transition-colors duration-200"
                onClick={goTo}
              >
                {textVariantButton.startsWith("Already have an account?") ? (
                  <>
                ...
                ) : textVariantButton.startsWith("Don't have an account?") ? (
                  <>
...
const BottomGradient = () => {
  return (
    <>
      ...
      <span className="group-hover/btn:opacity-100 blur-sm block transition duration-500 opacity-0 absolute h-px w-1/2 mx-auto -bottom-px inset-x-10 bg-gradient-to-r from-transparent via-orange-600 to-transparent" />
    </>
  );
};
